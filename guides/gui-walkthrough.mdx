---
title: "GUI Walkthrough: Every Control in pdf2wordx"
sidebarTitle: "GUI Walkthrough"
description: "A complete tour of the pdf2wordx interface — every button, label, and entry field explained with its purpose and behavior."
---
...
<Accordion title="Convert Button — &quot;Convertir&quot;">
  ...
  **Command:** `App.convertFile()` → `Funcs._convertFile(buttons)` (async, in background thread)
  ...
  Upon completion, both the "Open File" and "Convert" buttons are disabled to prevent duplicate conversions. The output directory path label is also updated to reflect the chosen save location.
